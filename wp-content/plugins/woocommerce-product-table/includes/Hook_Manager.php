...
class Hook_Manager extends \WP_Scoped_Hooks {
    ...
    public function register() {
        ...
        $this->add_filter( 'woocommerce_add_to_cart_form_action', array( __CLASS__, 'add_to_cart_form_action' ) );
    ...
    public static function add_to_cart_form_action( $url ) {
        // Set the add to cart form action to the current page, post or term. This helps return the customer to the product table rather
        // than the single product page in certain circumstances (e.g. adding to cart from the responsive modal).
        // This will only work for standard loaded tables.
    ...
    public function variations_dropdown_set_variation_image_props( $variation_data, $product, $variation ) {
        ...
        if ( empty( $variation_data['image']['caption'] ) ) {
            $variation_data['image']['caption'] = \trim( \strip_tags( \WCPT_Util::get_product_name( $product ) ) );
        }
    ...
    public static function product_addons_reset_display_hooks() {
        if ( isset( $GLOBALS['Product_Addon_Display'] ) && false === \has_action( 'woocommerce_before_add_to_cart_button', array( $GLOBALS['Product_Addon_Display'],
                'display' ) ) ) {
    ...
    public static function product_addons_select_template( $located, $template_name, $args, $template_path, $default_path ) {
        if ( 'woocommerce-product-addons' === $template_path ) {
            ...
            $template = \WCPT_Util::get_template_path() . $template_name;
    ...
    public static function woocommerce_variable_add_to_cart() {
        ...
        <form class="variations_form cart" action="<?php echo \esc_url( \apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo \esc_attr( $product->get_id() ); ?>" data-product_variations="<?php echo $variations_attr; ?>">
                ...
                <div class="variations">
                    <?php foreach ( $variation_attributes as $attribute_name => $options ) : ?>
                        ...
                        \wc_dropdown_variation_attribute_options( array(
                            ...
                            'id'               => $product->get_id() . '_' . \sanitize_title( $attribute_name ),
                            'product'          => $product,
                            'selected'         => $selected,
