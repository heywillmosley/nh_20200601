...
use Barn2\Plugin\WC_Product_Table\Hook_Manager,
    Barn2\Plugin\WC_Product_Table\Table_Cache,
...
class WC_Product_Table {
    ...
    public function get_table( $output = 'object' ) {
        ...
        if ( ! $this->table_initialised ) {
            ...
            Frontend_Scripts::register_table_scripts( $this->args );
    ...
    public function update( array $new_args ) {
        ...
        if ( $this->args->lazy_load && array_intersect( $args_changed, array( 'status', 'year', 'month', 'day', 'category', 'tag', 'term', 'cf', 'exclude',
                'include', 'exclude_category', 'rows_per_page', 'product_limit', 'sort_by', 'sort_order', 'search_term', 'search_filters' ) ) ) {
