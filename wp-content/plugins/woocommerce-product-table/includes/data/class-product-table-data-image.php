...
class Product_Table_Data_Image extends Abstract_Product_Table_Data {
    ...
    public function get_data() {
        ...
        if ( $attachment_id ) {
            ...
            // Caption fallback
            $atts['data-caption'] = empty( $atts['data-caption'] ) ? trim( strip_tags( WCPT_Util::get_product_name( $this->product ) ) ) : $atts['data-caption'];

            // Alt fallbacks
            $atts['alt'] = empty( $atts['alt'] ) ? $atts['data-caption'] : $atts['alt'];
            $atts['alt'] = empty( $atts['alt'] ) ? $atts['title'] : $atts['alt'];
            $atts['alt'] = empty( $atts['alt'] ) && $this->product ? trim( strip_tags( WCPT_Util::get_product_name( $this->product ) ) ) : $atts['alt'];
