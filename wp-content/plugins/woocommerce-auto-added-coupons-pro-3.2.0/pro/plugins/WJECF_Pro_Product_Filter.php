...
if ( defined( 'ABSPATH' ) && ! class_exists( 'WJECF_Pro_Product_Filter' ) ) {
	...
	class WJECF_Pro_Product_Filter extends Abstract_WJECF_Plugin {
		...
		function assert_coupon_is_valid( $coupon ) {
			$custom_fields = $coupon->get_meta( '_wjecf_custom_fields' );

			if ( empty( $custom_fields ) ) {
				return;
			}

			$custom_fields_and = $coupon->get_meta( '_wjecf_custom_fields_and' ) == 'yes';
		...
		private function product_matches_custom_field( $product, $cf ) {
			//If product has a getter we must use it.
			$function = 'get' . ( '_' === substr( $cf['key'], 0, 1 ) ? '' : '_' ) . $cf['key'];
			if ( is_callable( array( $product, $function ) ) ) {
				$metavalue = $product->{$function}();
			} else {
				$metavalue = $product->get_meta( $cf['key'] );
			}

			if (is_bool( $metavalue ) ) $metavalue = $metavalue ? 'yes' : 'no';
			if ( is_string( $metavalue ) || is_numeric( $metavalue ) ) {
				$lines = preg_split( '/\r\n|[\r\n]/', $cf['value'] );
				foreach ( $lines as $line ) {
					if ( false !== $metavalue && $this->smart_match( $line, (string) $metavalue ) ) {
						return true;
					}
				}
			}
		...
		private function smart_match( $match_string, $subject ) {
			...
			if ( substr( $match_string, 0, 1 ) == '/' ) {
				try {
					...
					//The user passed an invalid regex pattern.
					return false;
				}
			}
		...
		public function on_woocommerce_coupon_options_usage_restriction() {
			...
			$coupon_custom_fields = $coupon->get_meta( '_wjecf_custom_fields', true );
					...
					<span class="wjecf-table" id="wjecf-table-custom-field">
						<span class="tr">
							...
							<span class="th"><?php _e( 'Value', 'woocommerce' ); ?>
								<?php
								echo WJECF_Admin_Html::wc_help_tip(
									__(
	...
	<?php
		...
		public function admin_coupon_meta_fields( $coupon ) {
			return array(
				'_wjecf_custom_fields_and' => 'yesno',
				'_wjecf_custom_fields'     => array( 'callback' => array( $this, 'get_wjecf_custom_fields' ) ),
			);
		}

		public function get_wjecf_custom_fields() {
			...
			return empty( $custom_fields ) ? null : $custom_fields; //null will delete the meta from the coupon
		}
